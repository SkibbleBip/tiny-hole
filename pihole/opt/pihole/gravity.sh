...
gravity_CheckDNSResolutionAvailable() {
  ...
  if timeout 4 nslookup "${lookupDomain}" &> /dev/null; then
    if [[ -n "${secs:-}" ]]; then
      echo -e "${OVER}  ${TICK} DNS resolution is now available\\n"
    fi
    return 0
...
gravity_DownloadBlocklistFromUrl() {
  ...
  case $BLOCKINGMODE in
    "IP-NODATA-AAAA"|"IP")
        ...
        ip="$(nslookup "${domain}" | awk 'match($0,/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/) {if(NR > 2) print $3;}')"
        # Check if this IP matches any IP of the system
        if [[ -n "${ip}" && $(grep -Ec "inet(|6) ${ip}" <<< "$(ip a)") -gt 0 ]]; then
          blocked=true
        fi;;
    "NXDOMAIN")
        if [[ $(nslookup "${domain}" | awk 'match($0,/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/) {if(NR > 2) print $3;}' | grep "NXDOMAIN" -c) -ge 1 ]]; then
          blocked=true
        fi;;
    "NULL"|*)
        if [[ $(nslookup "${domain}" | awk 'match($0,/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/) {if(NR > 2) print $3;}'  | grep "0.0.0.0" -c) -ge 1 ]]; then
          blocked=true
        fi;;
   esac
  ...
  if [[ "${blocked}" == true ]]; then
    ...
    ip=$(nslookup "${domain}" "${ip_addr}" | awk 'match($0,/[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/) {if(NR > 2) print $3;}' )
