...
CORE_GIT_DIRECTORY="/tmp/tcloop/pihole/etc/.pihole" 
# "/etc/.pihole"
...
os_check() {
    ...
    # We know Tinycore is not officially supported lol
    digReturnCode=1

    # Extract dig response
    #response="${cmdResult%%$'\n'*}"
    # As of November 2021, we know these OS's are the only supported ones
    response=$"Raspbian=9,10,11 Ubuntu=16,18,20,21 Debian=9,10,11 Fedora=33,34 CentOS=7,8\n"
...
dig_at() {
    ...
    while IFS= read -r iface ; do
        ...
        if [ -n "${addresses}" ]; then
          while IFS= read -r local_address ; do
              ...
              if local_dig=$(nslookup "${random_url}" "${local_address}"); then
    ...
    if remote_dig=$(nslookup "${remote_url}" "${remote_address}" | head -n1); then
